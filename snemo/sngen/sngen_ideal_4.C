...
int main(int argc,char *argv[]){
  ...
  // Voltages
  const double vWire = 500.;

  float energy = 100; // in eV
  TString particle = "e-";
  ...
  for(Int_t itrk=0;itrk<Ntrk;itrk++){
	...
	continue;
      ...
      if(nclu>=MAXCLS) break;
		...
		Int_t nion;
		aval->GetAvalancheSize(nelec, nion);
		cout << "electrons in aval: " << nelec << " , ions in aval: " << nion << endl;
		ViewSignal* signalView = new ViewSignal();
		signalView->SetSensor(sensor);
		signalView->PlotSignal("s");
		//electron=sensor->GetElectronSignal("s",1.);
		sensor->ClearSignal();
		
		TH1D* sig_hists = signalView->GetHistogram();
	    //sig_hists->SaveAs("hist.root");
		integral = sig_hists->Integral();
		
		//cout << "electron: " << electron << endl;
		cout << "signal integral: " << integral << " fC = " << integral/1.6E-4 << " electrons" << endl;
		gain=integral/1.6E-4;
