...
#define  MAXCLS 200
	...
	for(Int_t itrk=0;itrk<Ntrk;itrk++){
		...
		b = gRandom->Uniform(XWallGap,XWallGap+0.5*rCell);
		//phi=gRandom->Uniform(2*TMath::PiOver2()+1.0,2*TMath::PiOver2()+1.06);
		phi=gRandom->Uniform((3*(TMath::PiOver2()))-0.211,3*(TMath::PiOver2())-0.111);
		theta=TMath::PiOver2();
...
//		}
		...
		for(irow=0; irow<nrows; irow++){
			for(icol=0; icol<ncols; icol++){
				...
				if(irow==2 || irow==3 || irow==4 || irow==5 || irow==6){
					continue;
				}
				if(irow==0 && icol==1){
					continue;
